...
/**
 * Number validator builder class
 *
 * @author Ezequiel Martin Apfel
 * @since  3-Feb-2009
 */
class NumberBuildValidator extends AbstractBuildValidator
{	
	/**
	...
	{	
		def validator = "<mx:NumberValidator source=\"{${id}}\" "+
			"property=\"${value}\" required=\"${!constraint.isBlank()}\""
		
		if (constraint.min)
			validator += " minValue=\"${constraint.min}\""
		
		if (constraint.max)
			validator += " maxValue=\"${constraint.max}\"" 
